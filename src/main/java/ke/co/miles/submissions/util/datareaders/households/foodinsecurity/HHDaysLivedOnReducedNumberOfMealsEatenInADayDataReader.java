 ...
 */
@Slf4j
public class HHDaysLivedOnReducedNumberOfMealsEatenInADayDataReader extends AbstractHouseholdDataReader {

    public HHDaysLivedOnReducedNumberOfMealsEatenInADayDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Days On Reduced Number Of Meals Eaten In A Day Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_REDUCE_MEAL_NUMBERS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_REDUCE_MEAL_NUMBERS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_REDUCE_MEAL_NUMBERS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Days On Reduced Number Of Meals Eaten In A Day from the Value String
                    log.trace("Reading in the Days On Reduced Number Of Meals Eaten In A Day Value from the Value String");
                    log.debug("Days On Reduced Number Of Meals Eaten In A Day = {}", parser.getString());
                    hdb.daysOnReducedNumberOfMealsEatenInADay(parser.getString());
                    ...
                    log.error("An invalid Days On Reduced Number Of Meals Eaten In A Day Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
