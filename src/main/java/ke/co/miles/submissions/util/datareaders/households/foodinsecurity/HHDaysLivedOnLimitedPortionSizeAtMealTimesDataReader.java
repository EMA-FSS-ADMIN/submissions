 ...
 */
@Slf4j
public class HHDaysLivedOnLimitedPortionSizeAtMealTimesDataReader extends AbstractHouseholdDataReader {

    public HHDaysLivedOnLimitedPortionSizeAtMealTimesDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Days On Limited Portion Size At Meal Times Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_LIMIT_PORTION_SIZE.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_LIMIT_PORTION_SIZE.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_LIMIT_PORTION_SIZE.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Days On Limited Portion Size At Meal Times from the Value String
                    log.trace("Reading in the Days On Limited Portion Size At Meal Times Value from the Value String");
                    log.debug("Days On Limited Portion Size At Meal Times = {}", parser.getString());
                    hdb.daysOnLimitedPortionSizeAtMealTimes(parser.getString());
                    ...
                    log.error("An invalid Days On Limited Portion Size At Meal Times Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
