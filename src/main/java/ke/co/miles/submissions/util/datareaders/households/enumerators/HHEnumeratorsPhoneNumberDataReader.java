 ...
 */
@Slf4j
public class HHEnumeratorsPhoneNumberDataReader extends AbstractHouseholdDataReader {

    public HHEnumeratorsPhoneNumberDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Phone Number Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_A_PHONE_NUMBER.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_A_PHONE_NUMBER.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_A_PHONE_NUMBER.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Phone Number from the Value String
                    log.trace("Reading in the Phone Number Value from the Value String");
                    log.debug("Phone Number = {}", parser.getString());
                    hdb.enumeratorsPhoneNumber(parser.getString());
                    ...
                    log.error("An invalid Phone Number Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
