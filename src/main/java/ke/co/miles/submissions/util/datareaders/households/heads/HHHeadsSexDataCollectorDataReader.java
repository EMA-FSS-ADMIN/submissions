 ...
 */
package ke.co.miles.submissions.util.datareaders.households.heads;
 ...
 */
@Slf4j
public class HHHeadsSexDataCollectorDataReader extends AbstractHouseholdDataReader {

    public HHHeadsSexDataCollectorDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Sex Of Household Head Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_SEX_HHH.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_SEX_HHH.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_SEX_HHH.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Sex Of the Household Head from the Value String
                    log.trace("Reading in the Sex Of the Household Head from the Value String");
                    log.debug("Sex Of Household Head = {}", parser.getString());
                    hdb.householdHeadsSex(parser.getString());
                    ...
                    log.error("An invalid Sex Of Household Head Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
