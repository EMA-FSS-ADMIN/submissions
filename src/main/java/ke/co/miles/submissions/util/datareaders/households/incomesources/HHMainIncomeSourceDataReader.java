 ...
 */
@Slf4j
public class HHMainIncomeSourceDataReader extends AbstractHouseholdDataReader {

    public HHMainIncomeSourceDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Main Income Source Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_MAIN_INCOME_SOURCE.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_MAIN_INCOME_SOURCE.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_MAIN_INCOME_SOURCE.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Main Income Source from the Value String
                    log.trace("Reading in the Main Income Source Value from the Value String");
                    log.debug("Main Income Source = {}", parser.getString());
                    hdb.mainIncomeSource(parser.getString());
                    ...
                    log.error("An invalid Main Income Source Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
