 ...
 */
@Slf4j
public class HHSweetPotatoesLossDataReader extends AbstractHouseholdDataReader {

    public HHSweetPotatoesLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Sweet Potatoes Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_SWEETPOTATO_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_SWEETPOTATO_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_SWEETPOTATO_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Sweet Potatoes Loss from the Value String
                    log.trace("Reading in the Sweet Potatoes Loss Value from the Value String");
                    log.debug("Sweet Potatoes Loss = {}", parser.getString());
                    hdb.sweetPotatoesLoss(parser.getString());
                    ...
                    log.error("An invalid Sweet Potatoes Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
