 ...
 */
@Slf4j
public class HHTomatoesLossDataReader extends AbstractHouseholdDataReader {

    public HHTomatoesLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Tomatoes Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_TOMATO_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_TOMATO_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_TOMATO_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Tomatoes Loss from the Value String
                    log.trace("Reading in the Tomatoes Loss Value from the Value String");
                    log.debug("Tomatoes Loss = {}", parser.getString());
                    hdb.tomatoesLoss(parser.getString());
                    ...
                    log.error("An invalid Tomatoes Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
