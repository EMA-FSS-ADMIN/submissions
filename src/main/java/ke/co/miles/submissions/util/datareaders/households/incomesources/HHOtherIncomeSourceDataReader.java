 ...
 */
package ke.co.miles.submissions.util.datareaders.households.incomesources;
 ...
 */
@Slf4j
public class HHOtherIncomeSourceDataReader extends AbstractHouseholdDataReader {

    public HHOtherIncomeSourceDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Other Income Source Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_OTHER_INCOME_SOURCES.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_OTHER_INCOME_SOURCES.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_OTHER_INCOME_SOURCES.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Other Income Source from the Value String
                    log.trace("Reading in the Other Income Source Value from the Value String");
                    log.debug("Other Income Source = {}", parser.getString());
                    hdb.otherIncomeSource(parser.getString());
                    ...
                    log.error("An invalid Other Income Source Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
