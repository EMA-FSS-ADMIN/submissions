 ...
 */
@Slf4j
public class HHSectionDataReader extends AbstractHouseholdDataReader {

    public HHSectionDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Section Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_A_SECTION.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_A_SECTION.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_A_SECTION.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Section Name from the Value String
                    log.trace("Reading in the Section Name from the Value String");
                    log.debug("Section Name = {}", parser.getString());
                    hdb.sectionName(parser.getString());
                    ...
                    log.error("An invalid Section Name Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
