 ...
 */
@Slf4j
public class HHDaysLivedOnRestrictedConsumptionByAdultsDataReader extends AbstractHouseholdDataReader {

    public HHDaysLivedOnRestrictedConsumptionByAdultsDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Days On Restricted Consumption By Adults Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_RESTRICT_CONSUMPTION.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_RESTRICT_CONSUMPTION.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_RESTRICT_CONSUMPTION.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Days On Restricted Consumption By Adults from the Value String
                    log.trace("Reading in the Days On Restricted Consumption By Adults Value from the Value String");
                    log.debug("Days On Restricted Consumption By Adults = {}", parser.getString());
                    hdb.daysOnRestrictedConsumptionByAdults(parser.getString());
                    ...
                    log.error("An invalid Days On Restricted Consumption By Adults Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
