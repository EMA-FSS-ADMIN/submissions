 ...
 */
package ke.co.miles.submissions.util.datareaders.households.locations.relative;
 ...
 */
@Slf4j
public class HHVillageNameDataReader extends AbstractHouseholdDataReader {

    public HHVillageNameDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Village Name Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_A_VILLAGE_NAME.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_A_VILLAGE_NAME.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name = {}", HHFormField.SECTION_A_VILLAGE_NAME.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Village Name from the Value String
                    log.trace("Reading in the Village Name from the Value String");
                    log.debug("Village Name = {}", parser.getString());
                    hdb.villageName(parser.getString());
                    ...
                    log.error("An invalid Village Name Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
