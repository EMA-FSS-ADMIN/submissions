 ...
 */
package ke.co.miles.submissions.util.datareaders.households.foodsources;
 ...
 */
@Slf4j
public class HHOtherFoodSourceDataReader extends AbstractHouseholdDataReader {

    public HHOtherFoodSourceDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Other Food Source Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_OTHER_FOOD_SOURCES.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_OTHER_FOOD_SOURCES.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_OTHER_FOOD_SOURCES.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Other Food Source from the Value String
                    log.trace("Reading in the Other Food Source Value from the Value String");
                    log.debug("Other Food Source = {}", parser.getString());
                    hdb.otherFoodSource(parser.getString());
                    ...
                    log.error("An invalid Other Food Source Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
