 ...
 */
package ke.co.miles.submissions.util.datareaders.households.livestock;
 ...
 */
@Slf4j
public class HHLivestockRearingStatusDataReader extends AbstractHouseholdDataReader {

    public HHLivestockRearingStatusDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Livestock Rearing Status Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_LIVESTOCK.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_LIVESTOCK.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_LIVESTOCK.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Livestock Rearing Status from the Value String
                    log.trace("Reading in the Livestock Rearing Status Value from the Value String");
                    log.debug("Livestock Rearing Status = {}", parser.getString());
                    hdb.hhRearsLivestock(parser.getString());
                    ...
                    log.error("An invalid Livestock Rearing Status Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
