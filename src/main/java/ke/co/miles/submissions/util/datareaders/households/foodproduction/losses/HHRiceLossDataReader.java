 ...
 */
@Slf4j
public class HHRiceLossDataReader extends AbstractHouseholdDataReader {

    public HHRiceLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Rice Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_RICE_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_RICE_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_RICE_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Rice Loss from the Value String
                    log.trace("Reading in the Rice Loss Value from the Value String");
                    log.debug("Rice Loss = {}", parser.getString());
                    hdb.riceLoss(parser.getString());
                    ...
                    log.error("An invalid Rice Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
