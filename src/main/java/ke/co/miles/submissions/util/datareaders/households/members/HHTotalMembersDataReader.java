 ...
 */
@Slf4j
public class HHTotalMembersDataReader extends AbstractHouseholdDataReader {

    public HHTotalMembersDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Total Members Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_TOTAL_HH_MEMEBERS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_TOTAL_HH_MEMEBERS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_TOTAL_HH_MEMEBERS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Total Members from the Value String
                    log.trace("Reading in the Total Members Value from the Value String");
                    log.debug("Total Members = {}", parser.getString());
                    hdb.totalMembers(parser.getString());
                    ...
                    log.error("An invalid Total Members Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
