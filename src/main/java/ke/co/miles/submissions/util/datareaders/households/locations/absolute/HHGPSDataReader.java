 ...
 */
package ke.co.miles.submissions.util.datareaders.households.locations.absolute;
 ...
 */
@Slf4j
public class HHGPSDataReader extends AbstractHouseholdDataReader {

    public HHGPSDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Household GPS Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_GPS_HOUSEHOLD.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_GPS_HOUSEHOLD.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_GPS_HOUSEHOLD.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Household's GPS from the Value String
                    log.trace("Reading in the Household's GPS Value from the Value String");
                    log.debug("Household GPS = {}", parser.getString());
                    hdb.householdGPS(parser.getString());
                    ...
                    log.error("An invalid Household GPS Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
