 ...
 */
@Slf4j
public class HHDistrictDataReader extends AbstractHouseholdDataReader {

    public HHDistrictDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering District Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_A_DISTRICT.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_A_DISTRICT.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_A_DISTRICT.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the District Name from the Value String
                    log.trace("Reading in the District Name from the Value String");
                    log.debug("District Name = {}", parser.getString());
                    hdb.districtName(parser.getString());
                    ...
                    log.error("An invalid District Name Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
