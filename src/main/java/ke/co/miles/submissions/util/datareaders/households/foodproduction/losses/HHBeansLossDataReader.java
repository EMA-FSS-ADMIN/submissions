 ...
 */
@Slf4j
public class HHBeansLossDataReader extends AbstractHouseholdDataReader {

    public HHBeansLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Beans Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_BEANS_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_BEANS_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_BEANS_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Beans Loss from the Value String
                    log.trace("Reading in the Beans Loss Value from the Value String");
                    log.debug("Beans Loss = {}", parser.getString());
                    hdb.beansLoss(parser.getString());
                    ...
                    log.error("An invalid Beans Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
