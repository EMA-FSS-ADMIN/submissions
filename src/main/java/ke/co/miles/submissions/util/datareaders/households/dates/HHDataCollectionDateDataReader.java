 ...
 */
package ke.co.miles.submissions.util.datareaders.households.dates;
 ...
 */
@Slf4j
public class HHDataCollectionDateDataReader extends AbstractHouseholdDataReader {

    public HHDataCollectionDateDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Data Collection Date Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_A_DATA_COLLECTIO_DATE.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_A_DATA_COLLECTIO_DATE.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_A_DATA_COLLECTIO_DATE.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Data Collection Date from the Value String
                    log.trace("Reading in the Data Collection Date Value from the Value String");
                    log.debug("Data Collection Date = {}", parser.getString());
                    hdb.dataCollectionDate(parser.getString());
                    ...
                    log.error("An invalid Data Collection Date Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
