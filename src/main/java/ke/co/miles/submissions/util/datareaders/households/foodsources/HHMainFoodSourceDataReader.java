 ...
 */
@Slf4j
public class HHMainFoodSourceDataReader extends AbstractHouseholdDataReader {

    public HHMainFoodSourceDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Main Food Source Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_MAIN_FOOD_SOURCE.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_MAIN_FOOD_SOURCE.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_MAIN_FOOD_SOURCE.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Main Food Source from the Value String
                    log.trace("Reading in the Main Food Source Value from the Value String");
                    log.debug("Main Food Source = {}", parser.getString());
                    hdb.mainFoodSource(parser.getString());
                    ...
                    log.error("An invalid Main Food Source Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
