 ...
 */
@Slf4j
public class HHGroundnutsLossDataReader extends AbstractHouseholdDataReader {

    public HHGroundnutsLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Groundnuts Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_GROUNDNUTS_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_GROUNDNUTS_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_GROUNDNUTS_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Groundnuts Loss from the Value String
                    log.trace("Reading in the Groundnuts Loss Value from the Value String");
                    log.debug("Groundnuts Loss = {}", parser.getString());
                    hdb.groundnutsLoss(parser.getString());
                    ...
                    log.error("An invalid Groundnuts Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
