 ...
 */
@Slf4j
public class HHRecordUUIDDataReader extends AbstractHouseholdDataReader {

    public HHRecordUUIDDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering UUID Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.UUID.getFieldName());
            if (parser.getString().equals(HHFormField.UUID.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.UUID.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the UUID from the Value String
                    log.trace("Reading in the UUID Value from the Value String");
                    log.debug("UUID = {}", parser.getString());
                    hdb.uuid(parser.getString());
                    ...
                    log.error("An invalid UUID Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
