 ...
 */
@Slf4j
public class HHMaleMembersDataReader extends AbstractHouseholdDataReader {

    public HHMaleMembersDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Male Members Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_HH_MALE_MEMBERS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_HH_MALE_MEMBERS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_HH_MALE_MEMBERS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Male Members from the Value String
                    log.trace("Reading in the Male Members Value from the Value String");
                    log.debug("Male Members = {}", parser.getString());
                    hdb.maleMembers(parser.getString());
                    ...
                    log.error("An invalid Male Members Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
