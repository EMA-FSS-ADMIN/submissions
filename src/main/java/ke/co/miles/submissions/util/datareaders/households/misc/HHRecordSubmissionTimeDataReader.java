 ...
 */
package ke.co.miles.submissions.util.datareaders.households.misc;
 ...
 */
@Slf4j
public class HHRecordSubmissionTimeDataReader extends AbstractHouseholdDataReader {

    public HHRecordSubmissionTimeDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Submission Time Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SUBMISSION_TIME.getFieldName());
            if (parser.getString().equals(HHFormField.SUBMISSION_TIME.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SUBMISSION_TIME.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Submission Time from the Value String
                    log.trace("Reading in the Submission Time Value from the Value String");
                    log.debug("Submission Time = {}", parser.getString());
                    hdb.submissionTime(parser.getString());
                    ...
                    log.error("An invalid Submission Time Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
