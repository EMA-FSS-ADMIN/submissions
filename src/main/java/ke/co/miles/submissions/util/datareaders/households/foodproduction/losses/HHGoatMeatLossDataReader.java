 ...
 */
@Slf4j
public class HHGoatMeatLossDataReader extends AbstractHouseholdDataReader {

    public HHGoatMeatLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Goat Meat Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_GOATMEAT_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_GOATMEAT_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_GOATMEAT_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Goat Meat Loss from the Value String
                    log.trace("Reading in the Goat Meat Loss Value from the Value String");
                    log.debug("Goat Meat Loss = {}", parser.getString());
                    hdb.goatMeatLoss(parser.getString());
                    ...
                    log.error("An invalid Goat Meat Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
