 ...
 */
@Slf4j
public class HHFoodInsecurityDataReader extends AbstractHouseholdDataReader {

    public HHFoodInsecurityDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Food Insecurity Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_FOOD_INSECURITY.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_FOOD_INSECURITY.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_FOOD_INSECURITY.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Food Insecurity from the Value String
                    log.trace("Reading in the Food Insecurity Value from the Value String");
                    log.debug("Food Insecurity = {}", parser.getString());
                    hdb.foodInsecurity(parser.getString());
                    ...
                    log.error("An invalid Food Insecurity Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
