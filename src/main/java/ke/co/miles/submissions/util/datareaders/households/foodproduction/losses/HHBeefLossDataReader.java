 ...
 */
@Slf4j
public class HHBeefLossDataReader extends AbstractHouseholdDataReader {

    public HHBeefLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Beef Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_BEEF_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_BEEF_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_BEEF_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Beef Loss from the Value String
                    log.trace("Reading in the Beef Loss Value from the Value String");
                    log.debug("Beef Loss = {}", parser.getString());
                    hdb.beefLoss(parser.getString());
                    ...
                    log.error("An invalid Beef Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
