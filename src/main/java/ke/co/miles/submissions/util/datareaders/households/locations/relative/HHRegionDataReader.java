 ...
 */
@Slf4j
public class HHRegionDataReader extends AbstractHouseholdDataReader {

    public HHRegionDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Region Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_A_REGION.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_A_REGION.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_A_REGION.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Region Name from the Value String
                    log.trace("Reading in the Region Name from the Value String");
                    log.debug("Region Name = {}", parser.getString());
                    hdb.regionName(parser.getString());
                    ...
                    log.error("An invalid Region Name Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
