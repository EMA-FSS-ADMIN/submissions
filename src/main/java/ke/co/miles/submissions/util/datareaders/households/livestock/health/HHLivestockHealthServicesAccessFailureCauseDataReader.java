 ...
 */
@Slf4j
public class HHLivestockHealthServicesAccessFailureCauseDataReader extends AbstractHouseholdDataReader {

    public HHLivestockHealthServicesAccessFailureCauseDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Animal Health Services Access Failure Cause Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_VET_SERVICES_FAILURE.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_VET_SERVICES_FAILURE.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_VET_SERVICES_FAILURE.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Animal Health Services Access Failure Cause from the Value String
                    log.trace("Reading in the Animal Health Services Access Failure Cause Value from the Value String");
                    log.debug("Animal Health Services Access Failure Cause = {}", parser.getString());
                    hdb.livestockHealthServicesAccessFailureCause(parser.getString());
                    ...
                    log.error("An invalid Animal Health Services Access Failure Cause Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
