 ...
 */
@Slf4j
public class HHCassavaLossDataReader extends AbstractHouseholdDataReader {

    public HHCassavaLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Cassava Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_CASSAVA_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_CASSAVA_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_CASSAVA_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Cassava Loss from the Value String
                    log.trace("Reading in the Cassava Loss Value from the Value String");
                    log.debug("Cassava Loss = {}", parser.getString());
                    hdb.cassavaLoss(parser.getString());
                    ...
                    log.error("An invalid Cassava Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
