 ...
 */
@Slf4j
public class HHRespondentsNameDataReader extends AbstractHouseholdDataReader {

    public HHRespondentsNameDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Respondent Name Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_RESPONDENT_NAME.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_RESPONDENT_NAME.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name = {}", HHFormField.SECTION_B_RESPONDENT_NAME.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Respondent Name from the Value String
                    log.trace("Reading in the Respondent Name from the Value String");
                    log.debug("Respondent Name = {}", parser.getString());
                    hdb.respondentsName(parser.getString());
                    ...
                    log.error("An invalid Respondent Name Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
