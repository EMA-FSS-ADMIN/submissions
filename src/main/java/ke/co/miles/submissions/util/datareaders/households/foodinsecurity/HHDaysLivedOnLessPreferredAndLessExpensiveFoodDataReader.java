 ...
 */
@Slf4j
public class HHDaysLivedOnLessPreferredAndLessExpensiveFoodDataReader extends AbstractHouseholdDataReader {

    public HHDaysLivedOnLessPreferredAndLessExpensiveFoodDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Days On Less Preferred And Less Expensive Food Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_LESS_DPENSIVE_FOOD.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_LESS_DPENSIVE_FOOD.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_LESS_DPENSIVE_FOOD.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Days On Less Preferred And Less Expensive Food from the Value String
                    log.trace("Reading in the Days On Less Preferred And Less Expensive Food Value from the Value String");
                    log.debug("Days On Less Preferred And Less Expensive Food = {}", parser.getString());
                    hdb.daysOnLessPreferredAndLessExpensiveFood(parser.getString());
                    ...
                    log.error("An invalid Days On Less Preferred And Less Expensive Food Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
