 ...
 */
package ke.co.miles.submissions.util.datareaders.households.respondents;
 ...
 */
@Slf4j
public class HHRespondentsSexDataCollectorDataReader extends AbstractHouseholdDataReader {

    public HHRespondentsSexDataCollectorDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Sex Of Respondent Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_SEX_RESPONDENT.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_SEX_RESPONDENT.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_SEX_RESPONDENT.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Sex Of the Respondent from the Value String
                    log.trace("Reading in the Sex Of the Respondent from the Value String");
                    log.debug("Sex Of Respondent = {}", parser.getString());
                    hdb.respondentsSex(parser.getString());
                    ...
                    log.error("An invalid Sex Of Respondent Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
