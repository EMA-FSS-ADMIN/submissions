 ...
 */
package ke.co.miles.submissions.util.datareaders.households.members;
 ...
 */
@Slf4j
public class HHFemaleMembersDataReader extends AbstractHouseholdDataReader {

    public HHFemaleMembersDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Female Members Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_HH_FEMALE_MEMBERS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_HH_FEMALE_MEMBERS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_HH_FEMALE_MEMBERS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Female Members from the Value String
                    log.trace("Reading in the Female Members Value from the Value String");
                    log.debug("Female Members = {}", parser.getString());
                    hdb.femaleMembers(parser.getString());
                    ...
                    log.error("An invalid Female Members Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
