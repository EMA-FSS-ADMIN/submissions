 ...
 */
@Slf4j
public class HHEPADataReader extends AbstractHouseholdDataReader {

    public HHEPADataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering EPA Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_A_EPA.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_A_EPA.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_A_EPA.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the EPA Name from the Value String
                    log.trace("Reading in the EPA Name from the Value String");
                    log.debug("EPA Name = {}", parser.getString());
                    hdb.epaName(parser.getString());
                    ...
                    log.error("An invalid EPA Name Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
