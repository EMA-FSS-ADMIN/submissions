 ...
 */
@Slf4j
public class HHMaizeLossDataReader extends AbstractHouseholdDataReader {

    public HHMaizeLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Maize Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_MAIZE_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_MAIZE_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_MAIZE_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Maize Loss from the Value String
                    log.trace("Reading in the Maize Loss Value from the Value String");
                    log.debug("Maize Loss = {}", parser.getString());
                    hdb.maizeLoss(parser.getString());
                    ...
                    log.error("An invalid Maize Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
