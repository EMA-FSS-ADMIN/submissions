 ...
 */
@Slf4j
public class HHFarmInputsInaccessibilityReasonsDataReader extends AbstractHouseholdDataReader {

  public HHFarmInputsInaccessibilityReasonsDataReader(JsonParser parser) {
    super(parser);
  }
  ...
  public boolean read(HouseholdDataBuilder hdb) {
    ...
    log.trace("Entering Farm Input Inaccessibility Reason Data Reader");
    ...
    try {
      ...
      // responsible for
      log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_FARM_INPUTS_NONE.getFieldName());
      if (parser.getString().equals(HHFormField.SECTION_B_FARM_INPUTS_NONE.getFieldName())) {

        // The target field name is equivalent to the field name that the processor is responsible
        // for
        log.trace("Field Name =  {}", HHFormField.SECTION_B_FARM_INPUTS_NONE.getFieldName());
        ...
        if (e == VALUE_STRING) {
          ...
          // Read in the Farm Input Inaccessibility Reason from the Value String
          log.trace("Reading in the Farm Input Inaccessibility Reason Value from the Value String");
          log.debug("Farm Input Inaccessibility Reason = {}", parser.getString());
          String reasons = parser.getString();
          if (reasons != null && !reasons.trim().isEmpty()) {
            for (String reason : reasons.split(" ")) {
              if (!reason.trim().isEmpty()) {
                hdb.farmInputInaccessibilityReason(reason);
              }
            }
          }
          ...
          log.error("An invalid Farm Input Inaccessibility Reason Value has been passed to the data processor");
        }
      } else {
        if (next != null) {
