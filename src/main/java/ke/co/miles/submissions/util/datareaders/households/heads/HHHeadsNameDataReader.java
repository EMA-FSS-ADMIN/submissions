 ...
 */
@Slf4j
public class HHHeadsNameDataReader extends AbstractHouseholdDataReader {

    public HHHeadsNameDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Household Head Name Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_HOUSELD_HEAD.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_HOUSELD_HEAD.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name = {}", HHFormField.SECTION_B_HOUSELD_HEAD.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Household Head Name from the Value String
                    log.trace("Reading in the Household Head Name from the Value String");
                    log.debug("Household Head Name = {}", parser.getString());
                    hdb.householdHeadsName(parser.getString());
                    ...
                    log.error("An invalid Household Head Name Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
