 ...
 */
@Slf4j
public class HHLivestockHealthServicesAccessDataReader extends AbstractHouseholdDataReader {

    public HHLivestockHealthServicesAccessDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Animal Health Services Access Status Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_VET_SERVICES.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_VET_SERVICES.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_VET_SERVICES.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Animal Health Services Access Status from the Value String
                    log.trace("Reading in the Animal Health Services Access Status Value from the Value String");
                    log.debug("Animal Health Services Access Status = {}", parser.getString());
                    hdb.livestockHealthServicesAccess(parser.getString());
                    ...
                    log.error("An invalid Animal Health Services Access Status Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
