 ...
 */
@Slf4j
public class HHVegetablesLossDataReader extends AbstractHouseholdDataReader {

    public HHVegetablesLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Vegetables Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_VEGETABLE_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_VEGETABLE_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_VEGETABLE_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Vegetables Loss from the Value String
                    log.trace("Reading in the Vegetables Loss Value from the Value String");
                    log.debug("Vegetables Loss = {}", parser.getString());
                    hdb.vegetablesLoss(parser.getString());
                    ...
                    log.error("An invalid Vegetables Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
