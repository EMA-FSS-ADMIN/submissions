 ...
 */
@Slf4j
public class HHEnumeratorsSexDataReader extends AbstractHouseholdDataReader {

    public HHEnumeratorsSexDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Sex Of Data Collector Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_A_SEX_OF_DATA_COLLECTOR.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_A_SEX_OF_DATA_COLLECTOR.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_A_SEX_OF_DATA_COLLECTOR.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Sex Of the Data Collector from the Value String
                    log.trace("Reading in the Sex Of the Data Collector from the Value String");
                    log.debug("Sex Of Data Collector = {}", parser.getString());
                    hdb.enumeratorsSex(parser.getString());
                    ...
                    log.error("An invalid Sex Of Data Collector Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
