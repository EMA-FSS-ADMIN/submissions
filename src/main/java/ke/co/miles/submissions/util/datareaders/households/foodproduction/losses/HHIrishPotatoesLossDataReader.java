 ...
 */
@Slf4j
public class HHIrishPotatoesLossDataReader extends AbstractHouseholdDataReader {

    public HHIrishPotatoesLossDataReader(JsonParser parser) {
        super(parser);
    }
    ...
    public boolean read(HouseholdDataBuilder hdb) {
        ...
        log.trace("Entering Irish Potatoes Loss Data Reader");
        ...
        try {
            ...
            log.trace("Checking if Field Name =  {}", HHFormField.SECTION_B_IRISHPOTATO_LOSS.getFieldName());
            if (parser.getString().equals(HHFormField.SECTION_B_IRISHPOTATO_LOSS.getFieldName())) {

                // The target field name is equivalent to the field name that the processor is responsible for
                log.trace("Field Name =  {}", HHFormField.SECTION_B_IRISHPOTATO_LOSS.getFieldName());
                ...
                if (e == VALUE_STRING) {
                    ...
                    // Read in the Irish Potatoes Loss from the Value String
                    log.trace("Reading in the Irish Potatoes Loss Value from the Value String");
                    log.debug("Irish Potatoes Loss = {}", parser.getString());
                    hdb.irishPotatoesLoss(parser.getString());
                    ...
                    log.error("An invalid Irish Potatoes Loss Value has been passed to the data processor");
                }
            } else {
                if (next != null) {
